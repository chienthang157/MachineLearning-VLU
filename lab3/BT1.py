...
def loadCsv(filename) -> pd.DataFrame:
    '''Code here'''
    df = pd.read_csv(filename)  # Đọc tệp CSV vào DataFrame
...
class KNNText:
    ...
    def __init__(self, k): # k là số điểm dữ liệu gần nhất
        '''Code here'''
        self.k = k  # Lưu số lượng k
        self.X_train = None  # Khởi tạo X_train
        self.y_train = None  # Khởi tạo y_train
    # hàm fit
